...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef<WaveUniforms>({
    ...
    enableMouseInteraction: new THREE.Uniform(enableMouseInteraction ? 1.0 : 0.0),
  ...
  useFrame(({ clock }) => {
    ...
    // Keep shader resolution aligned with the real GL drawing buffer.
    const bufferWidth = gl.domElement.width;
    const bufferHeight = gl.domElement.height;
    if (
      u.resolution.value.x !== bufferWidth ||
      u.resolution.value.y !== bufferHeight
    ) {
      u.resolution.value.set(bufferWidth, bufferHeight);
    }
    ...
    u.enableMouseInteraction.value = enableMouseInteraction ? 1.0 : 0.0;
